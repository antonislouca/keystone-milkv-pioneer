...
static int generic_extensions_init(struct sbi_hart_features *hfeatures) {
  if (generic_plat && generic_plat->extensions_init)
    return generic_plat->extensions_init(generic_plat_match, hfeatures);
...
static int generic_pmu_init(void) { return fdt_pmu_setup(fdt_get_address()); }
...
struct sbi_platform platform = {
    ...
    .platform_version = SBI_PLATFORM_VERSION(CONFIG_PLATFORM_GENERIC_MAJOR_VER,
